...
if ((Test-Path -LiteralPath $srcTool) -and -not (Test-Path -LiteralPath $toolDir)) {
    Say "copying PackageDeployertool (this takes a moment) ..."
    Copy-Item -LiteralPath $srcTool -Destination $InstallDir -Recurse -Force
    Say "copied the Package Deployer tool"
}
